...
import com.motadata.NMSLiteUsingVertex.messaging.ZmqConfig;
...
public class Main
{
    ...
    public static void main(String[] args)
    {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() ->
        {
            ...
            // close Postgrel pool gracefully
            DatabaseClient.closePool();

            // close zmqSocket gracefully
            ...
            vertx.close(responce ->
            {
                ...
                {
                    LOGGER.severe("Failed to close Vert.x: " + responce.cause().getMessage());
                }
            });
        }));
    }
    ...
    private static Future<Void> deployVerticles()
    {
        ...
        .compose(v -> deployVerticle(ZmqMessenger.class.getName(), new DeploymentOptions().setInstances(ZMQ_MESSENGER_VERTICLE_INSTANCES)))
    ...
    private static Future<Void> deployVerticle(String className, DeploymentOptions options)
    {
        ...
        vertx.deployVerticle(className, options, responce ->
        {
            if (responce.succeeded())
            {
                promise.complete();
