...
public class Discovery
{
    ...
    public static void handleRunDiscovery(RoutingContext ctx)
    {
        ...
        {
            LOGGER.warning("Invalid discovery id received: " + payload.getValue(DISCOVERY_ID_KEY));
        ...
        .compose(discoveryRecord ->
        {
            if (discoveryRecord==null)
            {
                LOGGER.warning("Discovery not found: " + discoveryId);
        ...
        .compose(reply ->
        {
            ...
            return QueryHandler.updateByField(DISCOVERY_TABLE, new JsonObject().put(DISCOVERY_STATUS_KEY, STATUS_COMPLETED), DISCOVERY_ID_KEY, discoveryId)
            .map(v -> reply.body());
        })
        .onSuccess(replyBody ->
        {
        ...
        .onFailure(err ->
        {
            ...
            {
                QueryHandler.updateByField(DISCOVERY_TABLE, new JsonObject().put(DISCOVERY_STATUS_KEY, STATUS_RESPONSE_FAIIED), DISCOVERY_ID_KEY, discoveryId).map(v -> err.getMessage())
                .onSuccess(errMsg ->
                {
