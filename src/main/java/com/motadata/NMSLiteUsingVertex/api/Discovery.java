...
public class Discovery
{
  ...
  public static Router getRouter()
  {
    ...
    // GET /api/discovery - get  discovery By Id
  ...
  private static void getDiscoveryByID(RoutingContext ctx)
  {
    ...
    ctx.vertx().<JsonObject>executeBlocking(promise ->
      {
        QueryHandler.getOneByField(DISCOVERY_TABLE, DISCOVERY_ID_KEY, credentialId)
  ...
  private static void getAllDiscovery(RoutingContext ctx)
  {
    ctx.vertx().<List<JsonObject>>executeBlocking(promise ->
      {
        QueryHandler.getAllWithJoin(DISCOVERY_TABLE, CREDENTIAL_TABLE, CREDENTIAL_ID_KEY)
          .onSuccess(promise::complete)
          .onFailure(promise::fail);
      })
  ...
  public static void handleRunDiscovery(RoutingContext ctx)
  {
    ...
    if (!payload.containsKey(DISCOVERY_ID_KEY) || !(payload.getValue(DISCOVERY_ID_KEY) instanceof Number))
    {
      ctx.response().setStatusCode(400).end(Utils.createResponse(STATUS_RESPONSE_FAIIED, "Invalid discovery id: must be an integer").encodePrettily());
      return;
    }
    ...
    ctx.vertx().<JsonObject>executeBlocking(promise ->
      ...
      .compose(eventBusMsg ->
      {
        if (eventBusMsg.body().toString().contains("Discovery failed"))
        {
          return Future.failedFuture(eventBusMsg.body().toString());
        }

        return ctx.vertx().executeBlocking(promise ->
        {
           QueryHandler.updateByField(DISCOVERY_TABLE, new JsonObject().put(DISCOVERY_STATUS_KEY, STATUS_COMPLETED), DISCOVERY_ID_KEY, discoveryId)
             .onSuccess(promise::complete)
             .onFailure(promise::fail);
      ...
      {
        ctx.response().setStatusCode(200).end(Utils.createResponse(STATUS_RESPONSE_SUCCESS, eventBusSuccessMsg).encodePrettily());
      })
      .onFailure(err ->
      {
        ...
        else
        {
          ctx.vertx().executeBlocking(promise ->
            {
               QueryHandler.updateByField(DISCOVERY_TABLE, new JsonObject().put(DISCOVERY_STATUS_KEY, STATUS_RESPONSE_FAIIED), DISCOVERY_ID_KEY, discoveryId)
                 .onSuccess(promise::complete)
                 .onFailure(promise::fail);
            })
            .map(v -> err.getMessage())
            .onSuccess(errMsg ->
            {
